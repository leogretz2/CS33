...
static char *next_p;      /* pointer to next block in iteration */
...
int mm_init(void) 
{
    ...
    heap_listp += DSIZE;
    next_p=heap_listp;                         /* next ptr starts at beginning */
...
/* 
 * next_fit - Find a fit for a block with asize bytes 
 */
static void *next_fit(size_t asize)
{ 
    /* next fit search */
...
static void *coalesce(void *bp) 
{
    ...
    next_p = bp; /* next ptr is now pointing to next block, iteration begins here */
    return bp;
}
